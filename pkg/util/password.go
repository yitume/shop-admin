...
// AAAmmgR0_FA4q9ltYct4sVQAwHqsOXUBLHsBnJDbqqE1t-ekCNAWszPC5cCu3phPZjJ_M-xj-_sWthvTBT8oFOKQsWM=$4$8ba7ce5e864fe86e1b7c0d2451b465a6bff8844a66da536b11a979bc$f18376fd717d9fc558bf5991c4351693451e0136e6cb7f6bce038169dfa71ae4
const (
	saltSize           = 16
	delmiter           = "$"
	stretchingPassword = 500
	saltLocalSecret    = "yitumelahgdofih"
)
...
func Verify(hashing string, pass string) error {
	...
	if (data["salt_secret"] + delmiter + data["interation_string"] + delmiter + has + delmiter + data["salt"]) == hashing {
		...
		model.Logger.Debug("util verify error", zap.String("pwd", hashing), zap.String("repwd", data["salt_secret"]+delmiter+data["interation_string"]+delmiter+has+delmiter+data["salt"]))
